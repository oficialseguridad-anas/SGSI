...
import type { ArchivoAdjuntoIncidente } from '../types';

interface Props {
  open: boolean;
  titulo: string;
  archivos: ArchivoAdjuntoIncidente[];
  onClose: () => void;
}
...
export function PrevisualizarSoportesIncidenteModal({ open, titulo, archivos, onClose }: Props) {
  const [archivoActivo, setArchivoActivo] = useState<ArchivoAdjuntoIncidente | null>(null);
  ...
  useEffect(() => {
    ...
    async function cargar() {
      ...
      setTextoPlano(null);
      try {
        const ruta = `/archivos-adjuntos-incidente/${archivoActivo!.id}/descargar/`;
  ...
  function nombreDe(archivo: ArchivoAdjuntoIncidente) {
  ...
  return (
    <Modal title={titulo} open={open} onCancel={onClose} footer={null} width={800} destroyOnHidden>
      {archivos.length === 0 ? (
        <Empty description="Este incidente no tiene soportes adjuntos." />
      ) : (
        <>
          {archivos.length > 1 && (
            <Segmented
            ...
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginTop: 16 }}>
                <Button
                  ...
                  onClick={() =>
                    descargarArchivo(
                      `/archivos-adjuntos-incidente/${archivoActivo.id}/descargar/`,
