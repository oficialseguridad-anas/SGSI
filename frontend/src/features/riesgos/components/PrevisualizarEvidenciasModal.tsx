...
import type { ArchivoAdjuntoTratamiento } from '../types';

interface Props {
  open: boolean;
  titulo: string;
  archivos: ArchivoAdjuntoTratamiento[];
  onClose: () => void;
}
...
export function PrevisualizarEvidenciasModal({ open, titulo, archivos, onClose }: Props) {
  const [archivoActivo, setArchivoActivo] = useState<ArchivoAdjuntoTratamiento | null>(null);
  ...
  useEffect(() => {
    ...
    async function cargar() {
      ...
      setTextoPlano(null);
      try {
        const ruta = `/archivos-adjuntos-tratamiento/${archivoActivo!.id}/descargar/`;
  ...
  function nombreDe(archivo: ArchivoAdjuntoTratamiento) {
  ...
  return (
    <Modal title={titulo} open={open} onCancel={onClose} footer={null} width={800} destroyOnHidden>
      {archivos.length === 0 ? (
        <Empty description="Este tratamiento no tiene soportes adjuntos." />
      ) : (
        <>
          {archivos.length > 1 && (
            <Segmented
            ...
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginTop: 16 }}>
                <Button
                  ...
                  onClick={() =>
                    descargarArchivo(
                      `/archivos-adjuntos-tratamiento/${archivoActivo.id}/descargar/`,
