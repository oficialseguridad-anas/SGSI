...
export function PrevisualizarSoporteModal({ open, titulo, seguimientoId, archivo, onClose }: Props) {
  ...
  useEffect(() => {
    if (!open || !seguimientoId || !archivo) return;
    ...
    async function cargar() {
      ...
      setTextoPlano(null);
      try {
        const ruta = `/seguimientos-indicador/${seguimientoId}/descargar/`;
  ...
  return (
    <Modal title={titulo} open={open} onCancel={onClose} footer={null} width={800} destroyOnHidden>
      {!archivo || !seguimientoId ? (
        <Empty description="Este seguimiento no tiene archivo de soporte." />
      ) : (
        <div>
            ...
            <>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginTop: 16 }}>
                <Button
                  ...
                  onClick={() => descargarArchivo(`/seguimientos-indicador/${seguimientoId}/descargar/`, nombre)}
                >
                  Descargar {nombre}
                </Button>
              </div>
