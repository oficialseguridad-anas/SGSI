...
import type { ArchivoAdjuntoSeguimiento } from '../types';

interface Props {
  open: boolean;
  titulo: string;
  archivos: ArchivoAdjuntoSeguimiento[];
  onClose: () => void;
}
...
export function PrevisualizarEvidenciasModal({ open, titulo, archivos, onClose }: Props) {
  const [archivoActivo, setArchivoActivo] = useState<ArchivoAdjuntoSeguimiento | null>(null);
  ...
  useEffect(() => {
    ...
    async function cargar() {
      ...
      setTextoPlano(null);
      try {
        const ruta = `/archivos-adjuntos-seguimiento/${archivoActivo!.id}/descargar/`;
  ...
  function nombreDe(archivo: ArchivoAdjuntoSeguimiento) {
  ...
  return (
    <Modal title={titulo} open={open} onCancel={onClose} footer={null} width={800} destroyOnHidden>
      {archivos.length === 0 ? (
        <Empty description="Este seguimiento no tiene evidencias adjuntas." />
      ) : (
        <>
          {archivos.length > 1 && (
            <Segmented
            ...
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginTop: 16 }}>
                <Button
                  ...
                  onClick={() =>
                    descargarArchivo(
                      `/archivos-adjuntos-seguimiento/${archivoActivo.id}/descargar/`,
