...
// Mismos colores que la columna "Estado de seguimiento" de la tabla de Indicadores.
const ESTILO = {
  alDia: { color: '#0ca30c', claro: '#6fd66f', etiqueta: 'Al día' },
  pendiente: { color: '#d03b3b', claro: '#f18b8b', etiqueta: 'Pendiente' },
};
const ORDEN: (keyof typeof ESTILO)[] = ['alDia', 'pendiente'];

export function GraficaEstadoSeguimientoIndicadores() {
  const { data, isLoading } = useQuery({ queryKey: ['indicadores'], queryFn: fetchIndicadores });
  const [vistaTabla, setVistaTabla] = useState(false);

  const conteos = useMemo(() => {
    const base = { alDia: 0, pendiente: 0 };
    for (const indicador of data?.results ?? []) {
      if (indicador.seguimiento_al_dia) base.alDia += 1;
      else base.pendiente += 1;
    }
    return base;
  }, [data]);

  const total = conteos.alDia + conteos.pendiente;
    ...
    () => ({
      ...
      tooltip: {
        ...
        formatter: (p: { name: string; value: number; percent: number }) =>
          `<strong>${p.value}</strong> indicadores · ${p.name} (${p.percent}%)`,
      },
      legend: {
        bottom: 2,
      ...
      graphic: [
        ...
        },
        {
          ...
          left: 'center' as const,
          top: '46%',
          style: { text: 'indicadores', fontSize: 13, fill: '#898781' },
        },
      ],
    }),
    [conteos, total],
    ...
    <div>
      ...
      {vistaTabla ? (
        ...
        <ReactECharts
          ...
          aria-label={`Estado de seguimiento: ${ORDEN.map((k) => `${ESTILO[k].etiqueta} ${conteos[k]}`).join(', ')}`}
        />
      )}
    </div>
  );
